...
class UserInterface:
    ...
    def __init__(self):
        ...
        login_button = tk.Button(bottom_frame, text="Login", width=10, command=self.start_login_window)
        login_button.pack(side='left', padx=5, pady=5)

        self.master_data = db.is_master_table_empty()
        if self.master_data:
            create_btn = tk.Button(bottom_frame, text="Create Vault", width=10, command=self.start_creating_master_password)
    ...
    def start_login_window(self):
        ...
        login_button = tk.Button(frame, text="Login", width=10, command=self.login_password_vault)
        login_button.pack(side='top', padx=5, pady=5)
        self.login_window.bind('<Return>', self.login_password_vault)

        self.center_position_window(self.login_window)

    def login_password_vault(self, _):
        ...
        if not self.master_data:
            ...
            self.login_window.destroy()
            self.start_creating_master_password()

    def start_creating_master_password(self):
        ...
        self.create_master_password_window.bind('<Return>', self.create_password_vault)
    ...
    def create_password_vault(self, _):
    ...
    def show_password_table(self):
        ...
        # Add the buttons
        self.show_passwords_button = tk.Button(master=bottom_frame, text="Show passwords",  width=15, command=self.show_or_hide_passwords)
        self.show_passwords_button.grid(row=0, column=0, padx=5, pady=5)

        add_credentials_button = tk.Button(bottom_frame, text="Add credentials", width=15, command=self.add_credentials_window)
        add_credentials_button.grid(row=0, column=1, padx=5, pady=5)

        change_master_key_button = tk.Button(bottom_frame, text="Change master key", width=15, command=self.change_master_password_window)
        change_master_key_button.grid(row=0, column=2, padx=5, pady=5)

        delete_all_button = tk.Button(bottom_frame, text="Delete all", width=15, command=self.delete_all_credentials)
        delete_all_button.grid(row=1, column=0, padx=5, pady=5)

        logout_button = tk.Button(bottom_frame, text="Logout", width=15, command=self.logout)
        logout_button.grid(row=1, column=1, padx=5, pady=5)

        self.password_table_window.update()
        self.center_position_window(self.password_table_window)

    def update_password_table(self, refresh=False, mask=True):
    ...
    def add_credentials_window(self):
        ...
        # self.add_entry_window.title = 'Add a new entry'
        self.add_entry_window.bind('<Return>', self.add_credentials)
        ...
        add_button = tk.Button(self.add_entry_window, text='Add', command=self.add_credentials)
        add_button.grid(row=3, column=0, padx=5, pady=5, sticky='ew')

        generate_password_button = tk.Button(self.add_entry_window, text='Generate Password', command=self.generate_password)
    ...
    def add_credentials(self, _):
        ...
        self.update_password_table(refresh=True)
    ...
    def change_master_password_window(self):
        ...
        self.change_master_passkey_window.bind('<Return>', self.change_master_password)
        ...
        change_password_button = tk.Button(self.change_master_passkey_window, text='Change Master Password', command=self.change_master_password)
    ...
    def change_master_password(self, _):
    ...
    def create_right_click_menu(self, event):
        ...
        right_click_menu.add_command(label='Update', command=lambda: self.update_credentials_window(row))
        right_click_menu.add_command(label='Delete', command=lambda: self.delete_credentials(row))
        right_click_menu.add_command(label='Copy', command=lambda: self.copy_credentials(row, column))
    ...
    def update_credentials_window(self, row):
        ...
        self.update_entry_window = tk.Toplevel()
        self.update_entry_window.iconbitmap(".\\icons\\key_icon.ico")
        self.update_entry_window.bind('<Return>', lambda: self.update_button_command(event=None, idx=entry_idx))

        update_website_label = tk.Label(self.update_entry_window, text="Updated website")
        update_website_label.grid(row=0, column=0, padx=5, pady=5, sticky='e')

        self.update_website_field = tk.Entry(self.update_entry_window)
        self.update_website_field.grid(row=0, column=1, padx=5, pady=5, sticky='ew')
        self.update_website_field.insert(0, entry[1])

        update_user_label = tk.Label(self.update_entry_window, text="Updated username")
        update_user_label.grid(row=1, column=0, padx=5, pady=5, sticky='e')

        self.update_user_field = tk.Entry(self.update_entry_window)
        self.update_user_field.grid(row=1, column=1, padx=5, pady=5, sticky='ew')
        self.update_user_field.insert(0, entry[2])

        update_password_label = tk.Label(self.update_entry_window, text="Updated password")
        update_password_label.grid(row=2, column=0, padx=5, pady=5, sticky='e')

        self.update_password_field = tk.Entry(self.update_entry_window)
        self.update_password_field.grid(row=2, column=1, padx=5, pady=5, sticky='ew')
        self.update_password_field.insert(0, entry[3])

        update_button = tk.Button(self.update_entry_window, text='Update', command=lambda: self.update_credentials(entry_idx))
        update_button.grid(row=3, column=0, padx=5, pady=5, sticky='ew')

        cancel_button = tk.Button(self.update_entry_window, text='Cancel', command=lambda: self.update_entry_window.destroy())
        cancel_button.grid(row=3, column=1, padx=5, pady=5, sticky='ew')

        self.center_position_window(self.update_entry_window)
    ...
    def update_credentials(self, idx):
        ...
        if res:
            ...
            self.update_entry_window.destroy()
            self.update_password_table(refresh=True)
        else:
    ...
    def copy_credentials(self, row, column):
    ...
    #     self.password_table.delete(*self.password_table.get_children())
    #     self.update_password_table()
    #     if self.show_passwords_button.cget('text') == "Hide passwords":
    #         self.show_passwords_button.configure(text="Show passwords")
    ...
    # def login_button_command(self, event):
    ...
    # def add_button_command(self, event):
    #     self.add_credentials()

    # def change_password_button_command(self, event):
